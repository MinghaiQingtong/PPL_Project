...
namespace TangtongMaple{
    ...
    bool ConstructFirstFarkasStatement(char *ch){
        ...
        local i,temp,temp2,S_eqnsMatrix,S_vector,C_vector,eqns,lambda,f,T;\
        S_eqnsMatrix:={};\
 ...
 */
    ...
    bool ConstructStdFormat(char *ch) {
        string str = "StdFormat:=proc(ineqns) \
                    local i, e, tmp; \
                    tmp:={}; \
                    for i from 1 to nops(ineqns) do\
                    e:={0<=rhs(ineqns[i])-lhs(ineqns[i])};\
                    tmp:=`union`(tmp, e);\
                    od;\
                    return tmp;\
                    end proc;\
    ...
    bool processStdFormat(MKernelVector &kv, char *maplestatement) {
        ConstructStdFormat(maplestatement);
        EvalMapleStatement(kv, maplestatement);
    }
